...
//            $this->db->like('kode_divisi',$search)
//                     ->like('nama_divisi',$search);
        }
...
//                     ->like('nama_divisi',$search);
    ...
    function get_paged_list($limit = 10, $offset = 0, $search="", $order='kode_divisi')
    {
        if(!empty($search))
        {
            $this->db->where($search,NULL,false);
//            $this->db->like('kode_divisi',$search)
//                     ->or_like('nama_divisi',$search);
        }
    ...
    // get persons with paging
    function get_paged_list_active($limit = 10, $offset = 0, $search="", $order='kode_divisi')
    {
        if(!empty($search))
        {
            $this->db->where($search,NULL,false);
