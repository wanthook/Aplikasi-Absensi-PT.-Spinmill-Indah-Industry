...
//                    }
                    //$str = $detail['kode'];
                    if((strtotime($detail['tanggal'])*1000)<  (strtotime($arr['tmk'])*1000))
                    {
                        $str = "IN";
                    }
                    else if(isset($detail['kode']) && $detail['kode']!="-" && $detail['kode']!="&nbsp;")
                    {
                        $str = $detail['kode'];
                    }
...
//            echo ($listA['hit_lemact']!=0)."<br>";
            if($listA['gp']>0 && $listA['libur']!="1")
            {
//                echo "kesini\n"; 
                $detInf['kode'] = "GP";
            }        
            else if($listA['hit_lemact']>0)
            {
    ...
    private function laporan_xls($periode, $karyawan, $divisi)
    {
        ...
        if(count($listKaryawan)>0)
        {
            foreach ($listKaryawan as $list)
            {
                ...
                $listAbsen = $this->list_absen($periode,$list->person_id);
//                print_r($listAbsen);
                if($listAbsen==false)
                    continue;
...
//        print_r($dataArr);
        if(count($dataArr)>0)
        {
            $div = $this->Mod_divisi->get_paged_list_active(10000)->result();
            
            
            
            foreach($div as $listDiv)
            {
                $tempData = array();
                
                foreach($dataArr as $listDataArr)
                {
                    //$tempData[] = $listDataArr;
                    if($listDataArr['divisi'] == $listDiv->id_divisi)
                    {
                        $tempData[] = $listDataArr;
                    }
                }
                ...
                $halaman[] = array(
                    "kode_divisi" => $listDiv->id_divisi,
                    "nama_divisi" => $listDiv->nama_divisi,
                    "data" => $tempData
                );
            }
        }
...
//    }
    ...
    function list_absen($periode,$person_id)
    {
        $ret = array();
        
        if(!empty($periode) && !empty($person_id))
        {
            /*
            * deklarasi $dateStart
            * digunakan untuk menampung tanggal periodik
            ...
            * tanggal awal dimulai pada tanggal 22 bulan sebelum bulan terpilih
            */
           $dateStart = date("Y-m-d",strtotime("-1 month ".$periode."-21"));
           ...
           $search = "(laporan_karyawan.tanggal between '".$dateStart."' and '".$dateEnd."') and laporan_karyawan.person_id='".$person_id."'";
           
           $data = $this->Mod_laporan->laporan_karyawan($search)->result_array();
